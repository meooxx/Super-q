...
export default class NewsItem extends Component{
	...
	getTitle() {
		...
		<div className="newsItem-title">
		  <a className="newsItem-titleLink" href={this.props.item.url ? this.props.item.url : 'https://news.ycombinator/item?id=' + this.props.item.id}>
    ...
    getSubtext() {
		{/* 
		  score, author, number of discuss
		*/}
		return (
		<div className='newsItem-subtext'>
		  {this.props.item.score} points by <a href={'https://news.ycombinator.com/user?id=' + this.props.item.by}>{this.props.item.by}</a>
			  {/* Moment.utc(this.props.item.time * 1000.fromNow() | {this.getCommentLink() */}
				  {Data.getNow() - this.props.item.time * 1000 }
		</div>
		)
